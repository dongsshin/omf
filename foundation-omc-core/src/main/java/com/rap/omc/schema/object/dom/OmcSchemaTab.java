...
public class OmcSchemaTab extends OmcSchemaSysRoot{
    ...
    public OmcSchemaTab(OmcSchemaSysRootVO vo) {
        super(vo);
        // TODO Auto-generated constructor stub
    }
    public OmcSchemaTab(String obid) {
    ...
    protected void create(Map map){
        ...
        OmcSchemaServiceUtils.createSystemTab(this.getVo());
    }
    /**
     * 
     * @param map
    ...
    protected void inActiviate(Map map){
        // TODO Auto-generated method stub
        OmcSchemaServiceUtils.inactivateSystemTab(this.getVo());
    }
    ...
    protected void delete(Map map){
        // TODO Auto-generated method stub
        OmcSchemaServiceUtils.deleteSystemTab(this.getVo());
    }
    ...
    protected void modify(Map map){
        // TODO Auto-generated method stub
        OmcSchemaServiceUtils.modifySystemTab(this.getVo());
    }
    ...
    protected void setFlags(){
        ...
        long flags = Bit.or(OmcSystemConstants.SYSTAB_FLAG_Default,OmcSystemConstants.SYSTAB_FLAG_Active);
    ...
    protected void setClassKind(){
        OmcSchemaTabLayoutVO thisVO = this.getVo();
        thisVO.setClassKinds(OmcSystemConstants.SYSKEY_KIND_Tab);
        this.setVo(thisVO);
    }
    @Override
    ...
    public static void getCommonSelectTemporarySql(StringBuffer sqlStrBuff, OmcSQLVariableParameter variableParameter){
        ...
        sqlStrBuff.append(OmcFoundationConstant.newline).append("      ,(select x.obid from psystab x where x.pnames = a.pnames) as obid");
        sqlStrBuff.append(OmcFoundationConstant.newline).append("from zsyslayouttab a");
        sqlStrBuff.append(OmcFoundationConstant.newline).append("where a.pkinds_str = 'Tab'");
    }
    public static void getCommonSelectSql(StringBuffer sqlStrBuff, OmcSQLVariableParameter variableParameter){
        ...
        sqlStrBuff.append(OmcFoundationConstant.newline).append("from psystab a");
    }
    ...
    public static List<OmcSchemaTabLayoutVO> getUploadList(){
        return(OmcSchemaServiceUtils.getTabListForUpload());
    }
    public static List<OmcSchemaTabLayoutVO> getInactiveList(){
        return(OmcSchemaServiceUtils.getInActiveTabListForUpload());
    }
    ...
    private  List<OmcSchemaRelationVO> getRelationListForComand(){
        return(OmcSchemaServiceUtils.getSchemaRelationList(this.getVo(),OmcSystemConstants.SYSREL_KIND_TabHasCommand,false));
    }
    private  List<OmcSchemaMenuVO> getCommandList(){
        return(OmcSchemaServiceUtils.getCommandList(this.getVo()));
    }
    private  List<OmcSchemaMenuVO> getUploadCommandList(){
        return(OmcSchemaServiceUtils.getCommandListWithSeperatedList(this.getVo().getSubObjectArrayList()));
    }
    private void applySubObjList(){
        
        List<OmcSchemaMenuVO> newList          = this.getUploadCommandList();
        List<OmcSchemaRelationVO> savedRelList = this.getRelationListForComand();
        OmcSchemaRelationTab2Command dom;
        for(OmcSchemaRelationVO savedVO : savedRelList){
            boolean isInactive = true;
            for(OmcSchemaMenuVO newVo : newList){
                if(savedVO.getToObid().equals(newVo.getObid())) {
                    isInactive = false;
                    if(!Bit.isInclude(savedVO.getFlags(), OmcSystemConstants.SYSREL_FLAG_Active))
                    {
                        dom = new OmcSchemaRelationTab2Command(savedVO);
        ...
        for(OmcSchemaMenuVO newVo : newList){
            ...
            if(isNotExists) 
            {
                ...
                dom = new OmcSchemaRelationTab2Command(relVo);
                Map<String, Object> map = new HashMap<String, Object>();

package com.rap.omc.framework.message.database;
...
public class DatabaseMessageSource extends AbstractMessageSource implements RefreshableMessageSource{
    ...
    private Map<String, Map<Locale, MessageFormat>> cachedMessageFormats = new HashMap();
    private Map<String, Map<Locale, MessageFormat>> tempCachedMessageFormats = new HashMap();
    ...
    {
        if (("preLoad".equals(loadType)) || ("lazyLoad".equals(loadType))) this.loadType = loadType;
        else
            throw new MessageException("Message load mode should be defined among preLoad,lazyLoad");
    }

    
    public void setDataSource(DataSource dataSource)
    {
        if (this.jdbcTemplate == null)/* 109 */ this.jdbcTemplate = new JdbcTemplate(dataSource);
    ...
    protected List<Map<String, Object>> readMessageListFromDatabase(String code, String locale)
    {
        if (this.jdbcTemplate != null) {
            DatabaseMessageQuery queryObj = new DatabaseMessageQuery(this.tableName, this.codeColumn,
                    this.localeColumn, this.messageColumn);
        ...
        throw new MessageException("Datasource must be defined");
    }
	...
	protected Map<String, Map<Locale, MessageFormat>> createTempCacheMessageMap(){
        ...
        Map<String, Map<Locale, MessageFormat>> tempCachedMessageFormats = new HashMap<String, Map<Locale, MessageFormat>>();
        for (Map<String, Object> messageInfo : messageList) {
            ...
            if (!(NullUtil.notNone(code)))/*     */ continue;
    ...
    protected MessageFormat getMessageFormat(String code, Locale locale){
        ...
        if ((isLazyLoad()) && (messageFormat == null))
        {
            List<Locale> localePriority = calculateLocalePriority(locale);
            for (Locale localLocale : localePriority) {

...
//    private StringBuilder sb = new StringBuilder();
   private static final Logger log = LoggerFactory.getLogger(RequestDumperFilter.class);

    
    private static final ThreadLocal<StringBuilder> sb = new ThreadLocal<StringBuilder>() {
        @Override
        protected StringBuilder initialValue() {
            log.debug("Creating SimpleDateFormat for Thread : " + Thread.currentThread().getName());
//            return new SimpleDateFormat("dd/MM/yyyy");
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        doLog(" characterEncoding", response.getCharacterEncoding());
        doLog("        bufferSize", Integer.toString( response.getBufferSize() ));
...
//        }
    ...
    private void printLog(){
        log.info(sb.get().toString());
        
    }

    private void doLog(String attribute, String value) {
//        StringBuilder sb = new StringBuilder(80);
        sb.get().append(Thread.currentThread().getName());
        sb.get().append(' ');
        sb.get().append(attribute);
        sb.get().append('=');
        sb.get().append(value);
        sb.get().append("\n");
//        log.info(sb.toString());
    ...
    private void doLogX(String attribute, String value) {
        StringBuilder sb = new StringBuilder(80);
